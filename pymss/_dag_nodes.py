...
def _resample(audio: np.ndarray, source_sr: int, target_sr: int) -> np.ndarray:
    if source_sr == target_sr or audio.size == 0:
        return audio
    # Lazy import: torchaudio is already a pymss dependency (used by ensemble).
    import torchaudio  # type: ignore

    waveform = torch.from_numpy(np.ascontiguousarray(audio)).float()
    if waveform.ndim == 1:
        waveform = waveform[None, :]
    resampled = torchaudio.functional.resample(waveform, source_sr, target_sr)
...
def _execute_invert_phase(ctx: NodeContext, inputs: dict[str, Any]) -> NodeResult:
    ...
    artifact = AudioArtifact(-audio.audio, audio.sample_rate, source_path=audio.source_path, stem_name=audio.stem_name)
    return NodeResult(outputs={0: artifact})
