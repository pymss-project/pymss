...
def _resample(audio: np.ndarray, src_sr: int, dst_sr: int) -> np.ndarray:
    ...
    return torchaudio.functional.resample(waveform, src_sr, dst_sr).numpy()
...
def _execute_eq(ctx: NodeContext, inputs: dict[str, Any]) -> NodeResult:
    ...
    # torchaudio expects [batch, channels, samples] or [channels, samples].
    tw = torch.from_numpy(np.ascontiguousarray(waveform)).float()
    if low_gain != 0:
        tw = torchaudio.functional.bass_biquad(tw, sr, low_gain, float(low_freq), 0.707)
    if mid_gain != 0:
        tw = torchaudio.functional.equalizer_biquad(tw, sr, float(mid_freq), mid_gain, mid_q)
    if high_gain != 0:
        tw = torchaudio.functional.treble_biquad(tw, sr, high_gain, float(high_freq), 0.707)
    return NodeResult(outputs={0: AudioArtifact(tw.numpy(), sr)})
